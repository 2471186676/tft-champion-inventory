...
function fillPool(){
    ...
    function addToPool(id, cost, amount){
        for(let x = 0; x < amount; x++){ champPool[cost].push(id) }
    }
}
...
document.onkeyup = (e) =>{
    if(e.key.toLowerCase() == "d"){ rerollEvent() };
}

function rerollEvent(e){
    clearBlock();
    // console.log(rollChance[levelSlider.value])
    createBlock(getRandom(rollChance[levelSlider.value]));
    createBlock(getRandom(rollChance[levelSlider.value]));
    createBlock(getRandom(rollChance[levelSlider.value]));
    createBlock(getRandom(rollChance[levelSlider.value]));
    createBlock(getRandom(rollChance[levelSlider.value]));
